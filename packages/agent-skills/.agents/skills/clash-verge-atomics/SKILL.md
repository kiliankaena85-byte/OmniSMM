...
description: >
  Атомарное управление, глубокая диагностика и сетевой инжиниринг клиента Clash Verge Rev
  и ядра Mihomo (Clash.Meta) в среде Windows/Linux. Используй этот скилл ВСЕГДА, когда пользователь
  упоминает Clash Verge, Verge Rev, Mihomo, Clash.Meta, проксирование трафика, правила маршрутизации
  (rules, rule-providers, geosite, geoip), TUN-режим (gVisor/system, Wintun), Fake-IP DNS, сбои доступа
  к российским сайтам (.ru, .su, .рф, panel.smmtoolbox.ru, happydesk), сброс конфигурации при автообновлении
  подписки, управление через Windows Named Pipe (\\.\pipe\verge-mihomo) или REST API контроллера.
  Скилл регламентирует 3-уровневый Profile Enhancement (Merge, Script, Rules), защиту от перезаписи,
  устранение ловушки `mode: global`, изоляцию суверенного трафика РФ и бессбойную горячую перезагрузку.
---
...
- **Суверенный периметр РФ**: гарантированный `DIRECT` маршрут для государственных, банковских и B2B сервисов РФ при сохранении защищенного VLESS/Shadowsocks туннелирования для внешнего мира.
