...
description: >
  Комплексный архитектурный скилл для контрактной эволюции API, обеспечения 100% обратной
  совместимости и управления жизненным циклом схем в платформе OmniSMM (Next.js 16 App Router,
  Server Actions, REST эндпоинты, Webhooks провайдеров и платёжных шлюзов). Используй этот скилл
  ВСЕГДА, когда планируется изменение аргументов или возвращаемых типов Server Actions, обновление
  схем валидации Zod, модификация публичных API (/api/v1, /api/v2), доработка форматов вебхуков
  провайдеров (Socgress, SmmLavka, Likemania и др.), добавление полей в DTO заказов, пользователей
  или биллинга, удаление устаревших эндпоинтов, внедрение RFC 8594 (Deprecation и Sunset заголовки),
  а также при проведении миграций по паттерну Dual-Read / Dual-Write. Скилл запрещает ломающие
  изменения (Breaking Changes) без переходного периода и гарантирует стабильность мобильного
  визарда, десктопных клиентов и внешних интеграций.
---
