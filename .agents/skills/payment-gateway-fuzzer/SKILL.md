...
description: "Используй этот скилл ВСЕГДА, когда Специализированный скилл
  фаззинга, стресс-тестирования и проверки отказоустойчивости платежных
  интеграций (ЮKassa, Robokassa, CryptoBot) платформы OmniSMM 1.0. Применяй этот
  скилл ВСЕГДА, когда создаются, модифицируются или тестируются: платежные
  вебхуки, инициализация платежей, зачисление баланса, возврат средств
  (refunds), дедупликация idempotencyKey в WalletOps, обработка частичных оплат
  и обработка сетевых сбоев платежных систем. Гарантирует защиту от
  Double-Crediting (двойного зачисления) и гонок конкурентных нотификаций. НЕ
  применять для верстки UI или клиентских стилей Tailwind."
---

# SKILL: payment-gateway-fuzzer — Стресс-тестирование и фаззинг платежных шлюзов

## Назначение и границы (Overview & Scope)
Скилл `payment-gateway-fuzzer` регламентирует фаззинг, стресс-тестирование и проверку отказоустойчивости платежных интеграций (ЮKassa, Robokassa, CryptoBot) платформы OmniSMM 1.0.
...
- 🛑 **ИНВАРИАНТ 1:** Fuzzing Sandbox Only: стресс-тестирование выполняется строго на тестовых ключах шлюзов.
- 🛑 **ИНВАРИАНТ 2:** Double-Crediting Immunity: параллельные вебхуки об оплате не должны вызывать повторное зачисление.
- 🛑 **ИНВАРИАНТ 3:** Price Tampering Guard: сумма платежа извлекается строго из локальной PaymentTransaction БД.
- 🛑 **ИНВАРИАНТ 4:** Timing-Safe Signatures: подписи вебхуков проверяются только через crypto.timingSafeEqual().
- 🛑 **ИНВАРИАНТ 5:** Idempotent Balance Credit: зачисление средств сопровождается строгим idempotencyKey.
