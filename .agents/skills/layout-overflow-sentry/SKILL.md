...
description: Используй этот скилл ВСЕГДА, когда Архитектурный скилл для
  выявления, предотвращения и автоматического устранения (Auto-Healer) поплывшей
  верстки, нежелательного горизонтального скролла, сплющивания элементов,
  некорректной DOM-вложенности и мобильных аномалий (iOS Auto-Zoom, 100vh
  jumping, Safe Area Insets) через CLI и Model Context Protocol (MCP). НЕ
  применять для низкоуровневой настройки PostgreSQL или очередей BullMQ.
---

# SKILL: layout-overflow-sentry — Детектор и Авто-исправитель верстки (RLS-2026)

## Назначение и границы (Overview & Scope)
Скилл `layout-overflow-sentry` регламентирует предотвращение горизонтального скролла, переполнения контейнеров и обрезания колонок в таблицах данных платформы OmniSMM.
...
### [LESSON-2026-09-14-SIL-AUTOSCROLL-PAYMENT] autoscroll-elimination-and-tunnel-gateway-verified
- **Trigger Condition:** Агрессивный срыв скролла вверх страницы при любом взаимодействии с чекаутом (Desktop & Mobile: ввод email, переключение чекбокса 152-ФЗ, изменение количества) и ошибка редиректа оплаты на localhost:3000 через туннели (SIL-2026).
- **Enforced Solution Pattern:**
  1. **Ликвидация паразитного автоскролла в чекаутах:**
     - Запрещено использовать нестабильные зависимости в `useEffect` для компонентов со скроллом (например, `useEffect(..., [onClose])`, где `onClose` передается как inline-стрелочная функция).
     - Колбэки закрытия и обработчики событий обязаны сохраняться в `useRef` (`onCloseRef.current = onClose`) или мемоизироваться через `useCallback`.
     - Скролл к форме чекаута `window.scrollTo({ top: 0 })` обязан быть защищен флагом `hasMountedScrollRef = useRef(false)`, проверкой вьюпорта `window.innerWidth >= 768` и пустым массивом зависимостей `[]`.
     - На мобильных устройствах (`< 768px`) скрытые десктопные компоненты обязаны быть изолированы от выполнения любых браузерных скроллов и мутаций истории `window.history.pushState`.
  2. **Стабилизация контейнеров мобильного визарда:**
     - Запрещено использовать `initial={{ height: 0 }}` в `motion.div` на шагах чекаута, вызывающее микроколлапсы верстки при ререндерах. Использовать чистый `opacity: 0` $\to$ `opacity: 1`.
     - Элементы шагов визарда обязаны содержать явные якорные DOM ID (`id="step-1"`, `id="step-2"`, `id="step-3"`, `id="step-4"`).
  3. **Сетевая адресация шлюзов оплаты (Tailscale & Tunnels):**
     - В `src/utils/get-base-url.ts` массив `ALLOWED_TUNNEL_SUFFIXES` обязан включать `.ts.net`, `.trycloudflare.com`, `.ngrok-free.app` и др.
     - Функция `isAllowedHost()` обязана валидировать суффиксы туннелей.
     - Для туннелей протокол принудительно фиксируется как `https://`.
- **Verified Date:** 2026-09-14

---

## Жесткие инварианты (Hard Invariants)
- 🛑 **ИНВАРИАНТ 1:** Zero Horizontal Scroll: запрещен неконтролируемый горизонтальный скролл на любых разрешениях.
- 🛑 **ИНВАРИАНТ 2:** Min-W-0 on Flex Children: flex-элементы с длинным текстом обязаны иметь класс min-w-0.
- 🛑 **ИНВАРИАНТ 3:** Table Viewport Fit: таблицы данных умещаются на 100% ширины экрана без раздутых колонок.
- 🛑 **ИНВАРИАНТ 4:** Truncate with Title: усеченный текст обязан сопровождаться атрибутом title или Tooltip.
- 🛑 **ИНВАРИАНТ 5:** Responsive Card Switch: на мобильных экранах (< 768px) таблицы сворачиваются в карточки.
