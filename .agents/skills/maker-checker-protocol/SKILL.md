...
description: Используй этот скилл ВСЕГДА, когда Инженерный протокол разделения
  ролей Maker-Checker (Создатель vs Независимый Ревизор) с эпистемической
  изоляцией контекста, запретом записи (Zero-Write Sandbox) и 5-векторной
  матрицей вето для платформы OmniSMM 1.0. НЕ применять для рутинного написания
  кода без процедуры ревью.
---

# Maker-Checker Protocol (Разделение ролей: Создатель vs Ревизор)

## Назначение и границы (Overview & Scope)
Скилл `maker-checker-protocol` регламентирует разделение ролей Создателя (Maker) и Независимого Ревизора (Checker) с эпистемической изоляцией контекста и 5-векторной матрицей вето в OmniSMM.
