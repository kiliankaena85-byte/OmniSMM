...
description: Используй этот скилл ВСЕГДА, когда Стандарты мобильной конверсии
  (CRO), тач-интерфейсов, степперов и чекаут-визардов для платформ OmniSMM 1.0.
  НЕ применять для настройки инфраструктуры серверов и сетевых шлюзов.
metadata:
  tags:
    - mobile-cro
    - wizard
