...
description: Используй этот скилл ВСЕГДА, когда Архитектурные паттерны
  отказоустойчивости (Resilience), изоляции сбоев и предотвращения каскадных
  аварий в платформе OmniSMM. Используй этот скилл ВСЕГДА, когда речь идет о
  внешних поставщиках SMM (SMM panels, Drip-Feed APIs), платежных шлюзах
  (ЮKassa, Robokassa, CryptoBot), Circuit Breaker (состояния Closed, Open,
  Half-Open), Bulkhead (отсеки изоляции пулов воркеров, лимитов соединений и
  очередей per-provider и per-tenant), обязательных сетевых таймаутах
  (AbortSignal.timeout, исключение зависших сокетов и микротасок), Graceful
  Degradation (деградация сервиса, кэширование отве. НЕ применять для локальных
  синхронных утилит без внешних сетевых вызовов.
---
