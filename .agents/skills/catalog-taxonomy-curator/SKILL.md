...
description: Используй этот скилл ВСЕГДА, когда Архитектурный скилл
  систематизации каталога, управления канонической таксономией и нормализации
  импорта услуг в платформе OmniSMM 1.0 (SMMplan / SMMflux). Используй этот
  скилл ВСЕГДА, когда проектируются, импортируются, синхронизируются или
  рефакторятся категории и услуги от внешних SMM-провайдеров. Скилл
  регламентирует жесткое каноническое дерево категорий (до 6-9 понятных разделов
  на соцсеть), запрещает создание сырых мусорных категорий в БД, обеспечивает
  автоматическое извлечение технических тегов ([Для закрытых каналов],
  [Таргетинг], [Tgstat], [Россия]) в структурированные атрибуты услуги (f. НЕ
  применять для верстки UI или финансовых транзакций.
---
