...
description: Используй этот скилл ВСЕГДА, когда Мульти-тенантная архитектура
  OmniSMM 1.0 (бренды SMMplan smmplan.pro и SMMflux smmflux.ru, масштабирование
  на N тенантов), жесткая изоляция данных и юридический барьер по ст. 54.1 НК
  РФ. Используй этот скилл ВСЕГДА, когда проектируются или модифицируются схемы
  Prisma (модели с обязательным полем tenantId, составные ключи
  @@unique([tenantId, ...])), Server Actions с контекстом бренда, резолвинг
  доменов в src/proxy.ts (Host header, кука x_admin_tenant, переключатель
  <GlobalSiteSwitcher />), кэширование (unstable_cache с тегами
  catalog-${tenantId}, префиксы Redis ${tenantId}:*. НЕ применять для стилизации
  изолированных компонентов без брендовых токенов.
---
