...
description: Используй этот скилл ВСЕГДА, когда Сквозной конвейер генеративного
  UI-дизайна — превращение идеи в wireframe, затем в фотореалистичный макет
  через Nano Banana (Gemini Flash Image) и в чистый React 19 / Tailwind 4 код
  через Google Stitch. НЕ применять для прямого написания серверных транзакций
  базы данных.
metadata:
  tags:
    - wireframe
...
Скилл `wireframe-nanobanana-stitch` регламентирует быстрое прототипирование интерактивных вайрфреймов и пользовательских путей (User Flow) для платформы OmniSMM.
...
- 🛑 **ИНВАРИАНТ 1:** Fast Iteration Cycle: создание кликабельного вайрфрейма за минимальное количество шагов.
- 🛑 **ИНВАРИАНТ 2:** User Flow Clarity: каждый шаг оформления заказа визуально понятен и прозрачен.
- 🛑 **ИНВАРИАНТ 3:** Low-Fidelity First: фокус на структуре информации и эргономике до финальной покраски.
- 🛑 **ИНВАРИАНТ 4:** Mobile Accessibility: вайрфреймы проверяются на удобство использования одной рукой.
- 🛑 **ИНВАРИАНТ 5:** Handoff Alignment: структура экранов вайрфрейма готова для переноса в production код.
