...
async function runBankGradeDbAudit() {
  ...
  try {
    ...
    // -------------------------------------------------------------------------
    const totalDriftRes = await db.$queryRaw<Array<{ count: bigint }>>`
    ...
    `;
    const totalDriftCount = Number(totalDriftRes[0]?.count ?? 0n);
    ...
    if (totalDriftCount === 0) {
      ...
      results.push({
        ...
        status: 'FAIL',
        details: `КРИТИЧЕСКИЙ СБОЙ: Обнаружено расхождение баланса и леджера у ${totalDriftCount} аккаунтов (из 4762)! Требуется протокол карантина/компенсации.`,
        metric: {
          totalDriftCount,
          sampleUsers: ledgerDrifts.map((d) => ({
            userId: d.id,
            email: d.email,
