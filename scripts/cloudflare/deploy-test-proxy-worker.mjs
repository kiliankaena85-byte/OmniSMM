...
async function cfRequest(endpoint, method = 'GET', body = undefined, contentType = undefined) {
...
async function main() {
  ...
  if (existing) {
    const u = await cfRequest(`/zones/${ZONE_ID}/workers/routes/${existing.id}`, 'PUT', {
  ...
  console.log('\n3. Cleaning up stale Page Rules for test.smmplan.pro...');
  const prRes = await cfRequest(`/zones/${ZONE_ID}/pagerules?status=active`);
  const prs = prRes.result || [];
  let removed = 0;
  for (const rule of prs) {
    const val = rule.targets?.[0]?.constraint?.value || '';
  ...
  try {
    const probe = await fetch('https://test.smmplan.pro/api/health', {
      headers: { 'User-Agent': 'SMMplan-Deploy-Probe/1.0' },
