/**
 * scripts/admin/audit-admin-core.ts
 *
 * Инженерный аудит Фазы 1 Административной панели OmniSMM 1.0.
 ...
 *    - Budget Laptop (1366x768) — критический тест Zero Horizontal Scroll!
 *    - Tablet (768x1024)
 * 3. Перехватывает ошибки консоли и гидратации React 19.
 * 4. Снимает скриншоты в .planning/admin_visuals/
 * 5. Генерирует отчет docs/audits/ADMIN_CORE_AUDIT_REPORT.md
 */
...
const VISUALS_DIR = path.resolve(process.cwd(), '.planning', 'admin_visuals');
const REPORT_PATH = path.resolve(process.cwd(), 'docs', 'audits', 'ADMIN_CORE_AUDIT_REPORT.md');
...
const ADMIN_SCREENS = [
  { id: 'dashboard', name: 'Главный дашборд', path: '/admin/dashboard' },
  { id: 'orders', name: 'Управление заказами', path: '/admin/orders' },
  { id: 'catalog', name: 'Каталог услуг', path: '/admin/catalog' },
  { id: 'providers', name: 'Провайдеры API', path: '/admin/providers' },
  { id: 'finance', name: 'Финансовый хаб', path: '/admin/finance' },
  { id: 'clients', name: 'Клиентская база CRM', path: '/admin/clients' }
];
...
async function getOrCreateOwnerJwt(): Promise<string> {
  ...
  let sessionId = 'admin_audit_session_' + Date.now();
  try {
    const session = await prisma.session.create({
      data: {
        userId,
        expiresAt,
        userAgent: 'admin-core-audit-runner',
        ipAddress: '127.0.0.1'
      }
    });
...
async function runAdminCoreAudit() {
  console.log('═══════════════════════════════════════════════════════════════════════');
  console.log('🏛️  OmniSMM 1.0 — Комплексный аудит Административной панели (Фаза 1)');
  console.log('    Проверка Zero Horizontal Scroll, RBAC, Таблиц и React 19');
  console.log('═══════════════════════════════════════════════════════════════════════\n');
  ...
  for (const vp of VIEWPORTS) {
    ...
    for (const screen of ADMIN_SCREENS) {
      ...
      try {
        ...
        console.log(`   ${icon} ${screen.name.padEnd(24)} | HTTP ${httpStatus} | Scroll: ${geom.scrollWidth}px (Overflow: ${geom.overflowPx}px) | Squashed: ${geom.squashed}`);
      } catch (err: any) {
  ...
  let reportContent = `# 🏛️ Отчет аудита Административной панели (Фаза 1: Core Operations)
...
1. **Zero Horizontal Scroll на ноутбуках 1366px:** ${results.filter((r) => r.width === 1366 && r.overflowPx === 0).length === ADMIN_SCREENS.length ? '🟢 **Все экраны идеально умещаются в ширину экрана 1366px без горизонтального скролла.**' : '⚠️ Обнаружены экраны с переполнением.'}
