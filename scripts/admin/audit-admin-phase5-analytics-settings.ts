/**
 * scripts/admin/audit-admin-phase5-analytics-settings.ts
 *
 * Инженерный аудит Фазы 5 Административной панели OmniSMM 1.0
 * (Аналитика, Маркетинг, Экономика, Настройки, Политики баланса и База знаний).
 * Запускает Chromium Playwright под ролью OWNER на живом инстансе (порт 3000):
 * 1. Проверяет экраны:
 *    - /admin/analytics (Аналитика и рентабельность)
 *    - /admin/marketing (Маркетинг и промокоды)
 *    - /admin/economics/recommendations (AI Ценовая оптимизация)
 *    - /admin/settings (Системные настройки платформы)
 *    - /admin/settings/balance-policies (Политики баланса и лимитов)
 *    - /admin/knowledge (База знаний и статьи)
 ...
 * 4. Снимает скриншоты в .planning/admin_visuals_phase5/
 * 5. Генерирует отчет docs/audits/ADMIN_PHASE5_ANALYTICS_SETTINGS_REPORT.md
 */
...
const VISUALS_DIR = path.resolve(process.cwd(), '.planning', 'admin_visuals_phase5');
const REPORT_PATH = path.resolve(process.cwd(), 'docs', 'audits', 'ADMIN_PHASE5_ANALYTICS_SETTINGS_REPORT.md');
...
const PHASE5_SCREENS = [
  { id: 'analytics_hub', name: 'Аналитика & Рентабельность', path: '/admin/analytics' },
  { id: 'marketing_hub', name: 'Маркетинг & Промокоды', path: '/admin/marketing' },
  { id: 'ai_pricing_economics', name: 'AI Ценовая оптимизация (Экономика)', path: '/admin/economics/recommendations' },
  { id: 'system_settings', name: 'Системные настройки OmniSMM', path: '/admin/settings' },
  { id: 'balance_policies', name: 'Политики баланса и лимиты', path: '/admin/settings/balance-policies' },
  { id: 'knowledge_hub', name: 'База знаний & Статьи', path: '/admin/knowledge' }
];
...
async function getOrCreateOwnerJwt(): Promise<string> {
  ...
  let sessionId = 'admin_phase5_session_' + Date.now();
  try {
    const session = await prisma.session.create({
      data: {
        userId,
        expiresAt,
        userAgent: 'admin-phase5-audit-runner',
        ipAddress: '127.0.0.1'
      }
    });
...
async function runPhase5Audit() {
  console.log('================================================================');
  console.log('🚀 OmniSMM 1.0 Admin Audit — Phase 5: Analytics, Settings & Econ');
  console.log('================================================================');
  ...
  for (const screen of PHASE5_SCREENS) {
...
function generateReport(results: ScreenAuditResult[]) {
  ...
  let md = `# Инженерный отчет: Аудит Административной панели OmniSMM 1.0 (Фаза 5)\n\n`;
  md += `**Дата проведения:** ${new Date().toISOString()}\n`;
  md += `**Кластер:** Аналитика, Маркетинг, Экономика, Системные настройки и База знаний\n`;
  ...
  md += `Все скриншоты сохранены в локальную директорию \`.planning/admin_visuals_phase5/\`:\n\n`;
  for (const r of results) {
  ...
  if (failedChecks === 0) {
    md += `✅ **Все 6 экранов кластера аналитики, маркетинга, экономики и настроек полностью соответствуют нормативам Zero-Scroll, WCAG 2.2 AA и OmniSMM 1.0.**\n`;
    md += `Административная панель OmniSMM 1.0 прошла комплексный Playwright-аудит по всем ключевым кластерам (30 экранов, 90 замеров геометрии).\n`;
  } else {
    md += `⚠️ **Обнаружены замечания (${failedChecks} проверок не прошли норматив). Требуется устранение замечаний.**\n`;
  }
