...
### 🔴 УРОК 13 — ioredis lazyConnect и семантика redis.set(..., 'NX') (2026-09-24)
**Что случилось:** В `redis.ts` клиент сконфигурирован с `lazyConnect: true`. Проверка готовности `(status === 'ready' || status === 'connecting')` в `p0-alert-debouncer.ts` ошибочно считала клиент готовым, когда Redis был выключен, накапливая команды в очереди. Кроме того, при `redis.set(key, '1', 'EX', ttl, 'NX')` значение `null` (ключ уже заблокирован) ошибочно не возвращало `false`, а проваливалось в in-memory fallback, вызывая дублирование алертов.
**Правило:**
1. Проверка готовности Redis в критических путях дебаунсинга и мьютексов должна быть СТРОГО `redis.status === 'ready'`.
2. Результат `redis.set(..., 'NX') === null` означает, что блокировка уже удерживается другим вызовом / предыдущим алертом: необходимо возвращать `false` (подавление), а не создавать локальный in-memory инцидент.

### 🔴 УРОК 14 — Маскирование URI с паролями, содержащими спецсимволы (@ и %40) (2026-09-24)
**Что случилось:** Регулярное выражение маскирования паролей в `sensitive-data-filter.ts` использовало шаблон `([^@\s]+)@`, который спотыкался на паролях, содержащих символ `@` или `%40`. Первый же `@` обрывал группу пароля, а остаток пароля вместе с хостом ошибочно принимался за email (например, `s***@cluster.net`), оставляя начало пароля открытым. Кроме того, схемы `mongodb+srv` и `http` не попадали под узкий regex.
**Правило:** Всегда использовать универсальный жадный захват `[^\/\s?#]+` до маски хоста `@(?:[a-zA-Z0-9_.-]+|\[[a-fA-F0-9:]+\])`, поддерживающий любые схемы `[a-z][a-z0-9+.-]*:\/\/` и бессхемные строки `user:pass@host`.

### 🔴 УРОК 15 — Мульти-тенантная изоляция логов персонала и IDOR (2026-09-24)
**Что случилось:** В `src/actions/admin/staff.ts` выборки `adminAuditLog.findMany` не фильтровались по `tenantId`, что приводило к отображению действий операторов SMMflux в дашборде SMMplan. Кроме того, отсутствовала проверка прав на просмотр персональных логов сотрудников ранга OWNER, что создавало уязвимость IDOR (BOLA).
**Правило:**
1. Все выборки логов персонала обязаны включать фильтр `tenantId` на основе `resolveAdminTenantContext`.
2. Запросы персональных логов сотрудников ранга OWNER от операторов нижестоящих ролей (SUPER_ADMIN, SUPPORT) обязаны блокироваться с `403 Forbidden`.
3. При отсутствии пересечения брендов между администратором и целевым сотрудником запрос обязан отклоняться.
...
## 1. 🏗️ Архитектурные решения (ADR)
 ...
 - **ADR-2026-39: Two-Tier High-Margin AI Decision Engine (Gemini System 2 + System 1 Jev-Analog) (OmniSMM 1.0 RAC-2026):**
  ...
  - *Решение:*
    1. **Двухуровневый тандем System 1 + System 2:**
       - **System 1 (Локальный аналог Jev — Von 395M / ModernBERT):** Запуск в Docker с задержкой 10–30 мс (0 ₽ затрат на API) для задач реального времени: милисекундный арбитраж провайдеров, валидация ссылок, антифрод, `MarginGuard` и триаж тикетов.
       - **System 2 (Google Gemini Flash/Pro по подписке):** Глубокий ночной анализ прайс-листов 6 поставщиков, кластеризация эквивалентных услуг, упаковка в «Золотой каталог» (VIP B2B офферы с наценкой 500–700%), синтез комплексных бизнес-пакетов, моделирование эластичности цен.
    2. **Silent Auto-Refill Sentinel:** Фоновый демон BullMQ, превентивно доливающий подписчиков через API поставщиков при списании $>2\%$ до того, как клиент зафиксирует просадку.
    3. **Спецификация:** Зафиксирована в `docs/specs/SPEC-2026-09-24-HIGH-MARGIN-AI-DECISION-ENGINE.md`.

 - **ADR-2026-38: Alert Debouncing, Sensitive Data Redaction, and Staff Isolation Remediation (OmniSMM 1.0 RAC-2026):**
  - *Контекст:* Выявлены 3 критических дефекта: дублирование P0-алертов при сбоях Redis, утечка открытых паролей со спецсимволами `@` в логах, и межбрендовая утечка аудит-логов персонала с риском IDOR.
  - *Решение:*
    1. **P0 Alert Debouncer Guard:** Строгая проверка `redis.status === 'ready'`, точная семантика `NX` (подавление при `null`), сохранение оффлайн-дельты `pendingOfflineDelta` с гарантированной синхронизацией `INCRBY` при реконнекте, защита активных окон тишины от LRU-вытеснения.
    2. **Universal Sensitive URI Redaction:** Внедрены универсальные регулярные выражения для схемных и бессхемных URI с жадным захватом паролей со спецсимволами (`@`, `%40`).
    3. **Staff Multi-Tenant & IDOR Shield:** Строгая фильтрация `adminAuditLog.findMany` по `tenantId` и `allowedTenants`, проверка иерархии ролей (OWNER-protection).
    4. **Стабилизация CI & tsc:** Исключение `.agents/**/*` и `artifacts/**/*` из `tsconfig.json`, устранение утечки V8 heap.
  - *Верификация:* 27/27 тестов PASS (`smart-alert-deduplication-and-dlq-triage.test.ts`, `sensitive-data-filter.test.ts`, `multitenant-staff-isolation.test.ts`), `npx tsc --noEmit` — 0 ошибок, 0 утечек секретов.
